...
float bfs_SSSP(int src, int n, stack<int> &visitStack, vector<int> &sigma, list<int> *pred, adjacency_list &adjList) {
    ...
    while (!visitQueue.empty()) {
        ...
        visitQueue.pop();
        visitStack.push(v);
        cout<<"debug 31.1: "<<v<<endl;
        // Closeness part aggregation.
        closeness += dist[v];
        //cout<<"debug 31.1.0: "<<v<<endl;
        // Check the neighbors w of v.
        for (vector<neighbor>::iterator it = adjList[v].begin(); it != adjList[v].end(); it++) {
            cout<<"debug 31.1.1: "<<it->target<<endl;
            int w = it->target;
            if(w < n && w > 0){
                // Node w found for the first time?
                cout<<"debug 31.1.2: "<<v<<" "<<w<<endl;
                if (dist[w] < 0) {
                    visitQueue.push(w);
                    dist[w] = dist[v] + 1;
                }
        ...
        cout<<"debug 31.2: "<<v<<endl;
        cout<<"visitq : "<<visitQueue.empty()<<endl;
    }    
    cout<<"hi ; "<<closeness<<endl;
    // Closeness part inversion.
    if (closeness!=0) {
        return 1.0 / closeness;
    } else {
    ...
    for (int src = 0; src < n; src++) { 
        ...
        while (!visitStack.empty()) {
            ...
            for (list<int>::iterator it = pred[w].begin(); it != pred[w].end(); it++) {
                ...
                cout<<"debug x5 : "<<v<<" "<<w<<endl;
                delta[v] += c;

            }
            cout<<"debug x2 : "<<src<<endl;
            // Node betweenness aggregation part.
            if (w != src) {
                cout<<"debug x3 : "<<src<<" "<<w<<endl;
                cout<<"size  is = "<<nodeBetweenness.size()<<endl;
                cout<<nodeBetweenness[4]<<endl;
                nodeBetweenness[w] += delta[w];
                cout<<"debug x4 : "<<src<<endl;
            }
        }
        cout<<"debug 22.3 : "<<src<<endl;
